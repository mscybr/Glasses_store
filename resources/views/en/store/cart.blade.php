                            ...
                            @foreach ($orders as $item)
                                ...
                                <tr class="tf-cart-item file-delete">
                                    <td class="tf-cart-item_product">
                                        <a href="{{route("product", $item->product->id)}}" class="img-box">
                                            <img src="{{asset("storage/images/$thumbnail")}}" alt="img-product">
                                        </a>
                                        <div class="cart-info">
                                            <a href="product-detail.html" class="cart-title link">{{$item->product->name}}</a>
                                            <div class="cart-meta-variant">{{$item->color}} / {{$item->size}}</div>
                                            <span class="remove-cart link remove" onclick="remove_item({{$item->id}})">Remove</span>
                                        </div>
                                    </td>
                                    <td class="tf-cart-item_price" cart-data-title="Price">
                                        @if (isRtl($selected_currency["currencySymbol"]))
                                            <div class="cart-price" id="price-{{$item->id}}">{{  number_format(( $price / $selected_currency["cost"]), 2, ".", ""). $selected_currency["currencySymbol"]}}</div>
                                        @else
                                            <div class="cart-price" id="price-{{$item->id}}">{{ $selected_currency["currencySymbol"] . number_format(($price / $selected_currency["cost"]), 2, ".", "")}}</div>
                                        @endif
                                    </td>
                                    <td class="tf-cart-item_quantity" cart-data-title="Quantity">
                                        <div class="cart-quantity">
                                            <div class="wg-quantity">
                                                ...
                                                <input type="text" name="number" value="{{ $item->amount }}" onchange="change_item_amount({{$item->id}}, {{$price / $selected_currency["cost"]}}, {{$selected_currency["currencySymbol"]}}, {{isRtl($selected_currency["currencySymbol"]) ? "rtl" : "ltr"}}, this)">
                                    ...
                                    <td class="tf-cart-item_total" cart-data-title="Total">
                                        @if (isRtl($selected_currency["currencySymbol"]))
                                            <div class="cart-total" id="total-price-{{$item->id}}">{{  number_format(( $price * $item->amount / $selected_currency["cost"]), 2, ".", ""). $selected_currency["currencySymbol"]}}</div>
                                        @else
                                            <div class="cart-total" id="total-price-{{$item->id}}">{{ $selected_currency["currencySymbol"] . number_format(($price * $item->amount / $selected_currency["cost"]), 2, ".", "")}}</div>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <form action="{{route("send_to_check_out")}}" method="post">
                        <div class="tf-page-cart-checkout">
                            ...
                            <div class="tf-cart-totals-discounts">
                                <h3>Subtotal</h3>
                                @if (isRtl($selected_currency["currencySymbol"]))
                                    <span class="total-value" dir="rtl">
                            ...
                            <div class="tf-cart-totals-discounts">
                                ...
                                @csrf
                                <select class="tf-dropdown-sort" data-bs-toggle="dropdown" name="address">
                                    <option selected disabled>Select a Billing Address</option>
                                    @foreach ( Auth::user()->billing_addresses as $address )
                                        <option  value={{$address->id}} class="select-item active">
                                            <span class="text-value-item">{{ $address->address }}</span>
                                        </option>
                                    @endforeach
                                </select>
                            </div>
...
<script>
    function remove_item(id){
        fetch("{{route("delete_cart")}}?item_id="+id);
    }
</script>
