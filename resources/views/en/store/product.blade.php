...
@endphp
            ...
            <div class="container">
                <div class="tf-breadcrumb-wrap d-flex justify-content-between flex-wrap align-items-center">
                    <div class="tf-breadcrumb-list">
                        <a href="{{route("shop")}}" class="text">Store</a>
                        <i class="icon icon-arrow-right"></i>
                        <a href="{{route("shop", ["Category" => $product->category->id])}}" class="text">{{ $product->category->categoryName }}</a>
                        @if ($product->subcategory)
                        <i class="icon icon-arrow-right"></i>
                        <a href="{{route("shop", ["Subcategory" => $product->subcategory->id])}}" class="text">{{ $product->subcategory->subcategoryName }}</a>
                        @endif
                        <i class="icon icon-arrow-right"></i>
        ...
        <!-- /breadcrumb -->
        <!-- default -->
        <form method="post" action={{route("store_order")}} id="submit-form">
            @csrf
            <input type="hidden" name="product_id" value={{$product->id}}>
            <input type="hidden" name="size" id="size-field">
            <input type="hidden" name="lens" id="lens-field">
                                        ...
                                        <div class="swiper-wrapper stagger-wrap">
                                            @foreach ($images as $thumb)
                                                ...
                                                <div class="swiper-slide stagger-item">
                                                    <div class="item">
                                                        <img class="lazyload" data-src="{{$image_src}}" src="{{$image_src}}" alt="">
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                                        ...
                                        <div class="swiper-wrapper" >
                                            @foreach ($images as $thumb)
                                                ...
                                                <div class="swiper-slide">
                                                    <a href="{{$image_src}}" target="_blank" class="item" data-pswp-width="770px" data-pswp-height="1075px">
                                                        <img class="tf-image-zoom lazyload" data-zoom="{{$image_src}}" data-src="{{$image_src}}" src="{{$image_src}}" alt="">
                                                    </a>
                                                </div>
                                            @endforeach
                                ...
                                <div class="tf-product-info-list other-image-zoom">
                                    ...
                                    <div class="tf-product-info-price">
                                        ...
                                        @if (isRtl($selected_currency["currencySymbol"]))
                                            <div class="price-on-sale" dir="rtl">{{  number_format((($product->price - ( $product->sale * $product->price )) / $selected_currency["cost"]), 2, ".", "")." ". $selected_currency["currencySymbol"]}}</div>
                                        @else
                                            <div class="price-on-sale">{{ $selected_currency["currencySymbol"] . number_format((($product->price - ( $product->sale * $product->price )) / $selected_currency["cost"]), 2, ".", "")}}</div>
                                        @endif
                                        @if ($product->sale > 0)
                                            @if (isRtl($selected_currency["currencySymbol"]))
                                                <div class="compare-at-price" dir="rtl">{{  number_format((($product->price) / $selected_currency["cost"]), 2, ".", "")." ".$selected_currency["currencySymbol"]}}</div>
                                            @else
                                                <div class="compare-at-price">{{ $selected_currency["currencySymbol"] . number_format((($product->price) / $selected_currency["cost"]), 2, ".", "")}}</div>
                                            @endif
                                            <div class="badges-on-sale"><span>{{intval($product->sale * 100)}}</span>% OFF</div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="tf-product-info-variant-picker">
                                        <div class="variant-picker-item">
                                            ...
                                            <div class="variant-picker-values">
                                                @foreach ( explode(",", $product->colors) as $color )
                                                    <input onchange="mirror_change('#color-field', this.value)" id="values-{{ $color }}" type="radio" name="color" value="{{ $color }}">
                                                    <label class="hover-tooltip radius-60" for="values-{{ $color }}" data-value="{{ $color }}">
                                        ...
                                        <div class="variant-picker-item">
                                            ...
                                            <div class="variant-picker-values">
                                                @foreach ( explode(",", $product->sizes) as $size )
                                                    <input onchange="mirror_change('#size-field', this.value)" type="radio" name="size" id="values-{{ $size }}" value="{{ $size }}">
                                        ...
                                        @if ( $product->lenses != "" )
                                            <div class="variant-picker-item">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div class="variant-picker-label">
                                                        Lens: <span class="fw-6 variant-picker-label-value"></span>
                                                    </div>
                                                </div>
                                                <div class="variant-picker-values">
                                                    @foreach ( explode(",", $product->lenses) as $lens )
                                                        <input onchange="mirror_change('#lens-field', this.value)" type="radio" name="lens" id="lens-values-{{$lens}}" value="{{$lens}}">
                                                        <label class="style-text" for="lens-values-{{$lens}}" data-value="{{$lens}}">
                                                            <p>{{$lens}}</p>
                                                        </label>
                                                    @endforeach
                                                </div>
                                    ...
                                    <div class="tf-product-info-quantity">
                                        ...
                                        <div class="wg-quantity">
                                            ...
                                            <input type="text" name="number" value="1" id="quantity">
                                            <span class="btn-quantity plus-btn">+</span>
                                        </div>
                                    </div>
                                    <div class="tf-product-info-buy-button">
                                        <form class="">
                                            <a onclick="document.querySelector('#submit-form').submit()" class="tf-btn btn-fill justify-content-center fw-6 fs-16 flex-grow-1 animate-hover-btn "><span>Add to cart -&nbsp;</span><span id="total-price" class="tf-qty-price">$8.00</span></a>
        ...
        <script>
            let price = {{ (($product->price - ( $product->sale * $product->price )) / $selected_currency["cost"]) }};
            function mirror_change(field, value){
                document.querySelector(field).value = value
            }
            setInterval(() => {
                mirror_change('#amount-field', document.querySelector("#quantity").value)
                document.querySelector("#total-price").innerHTML =( price * document.querySelector("#quantity").value).toFixed(2);
            }, 1000);
        </script>
